...
function AwardedLoot:editWinner(checksum, winner, announce)
    ...
    if (announce) then
        ...
        if (GL.BoostedRolls:enabled() and GL:higherThanZero(AwardEntry.BRCost)) then
            awardMessage = string.format("%s a été gagné par %s for %s points. Congrats!",
                AwardEntry.itemLink,
                winner,
                AwardEntry.BRCost
            );
        elseif (AwardEntry.GDKPCost and AwardEntry.GDKPCost > 0) then
            awardMessage = string.format("%s a été gagné par %s for %sg. Congrats!",
                AwardEntry.itemLink,
                winner,
                AwardEntry.GDKPCost
            );
        else
            awardMessage = string.format("%s a été gagné par %s. Congrats!",
                AwardEntry.itemLink,
                winner
            );
