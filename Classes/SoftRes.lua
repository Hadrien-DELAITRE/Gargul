...
function SoftRes:handleWhisperCommand(_, message, sender)
    ...
    if (GL:empty(Reserves)) then
        GL:sendChatMessage(
            "Il semble que tu n'a rien soft-reservé encore. Vérifie le site de soft-res ou demande au ML.",
            "WHISPER", nil, sender
        );
...
function SoftRes:import(data, openOverview)
    ...
    if (not GL.User.isInGroup
        ...
        if (GL.Settings:get("SoftRes.enableWhisperCommand", true)
            ...
            GL:sendChatMessage(
                string.format("J'ai importé la soft-resa dans Gargul. Whispez moi !sr pour vérifier vos réservations !"),
                "GROUP"
            );
        end
    end
...
function SoftRes:postMissingSoftReserves()
    ...
    -- Post the link in the chat for all group members to see
    GL:sendChatMessage(
        "Il manque les soft-resa de : " .. table.concat(PlayerNames, ", "),
        "GROUP"
    );
